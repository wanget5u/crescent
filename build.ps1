...
$game_files = Get-ChildItem -Path "src" -Filter *.c -Recurse
...
foreach ($file in $game_files) {
    $rel_path = $file.FullName.Replace($srcPath, "").TrimStart('\', '/')
    $obj_rel_path = [System.IO.Path]::ChangeExtension($rel_path, ".o")
    $objPath = Join-Path -Path "build" -ChildPath $obj_rel_path

    $objDir = Split-Path -Path $objPath -Parent
    if (!(Test-Path $objDir)) {
...
if ($needs_linking -or !(Test-Path "bin/crescent.exe")) {
    ...
    $all_objs = Get-ChildItem -Path "build" -Filter *.o -Recurse | ForEach-Object { $_.FullName }
